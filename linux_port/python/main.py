...
from platform_compat  import (
    get_engine_lib_name,
    get_screen_size as detect_screen_size,
    get_webcam_backend,
    has_gui_session,
    print_runtime_notes,
)
...
def main():
    ...
    backend = get_webcam_backend()
    cap = cv2.VideoCapture(WEBCAM_IDX, backend)
    if not cap.isOpened():
        cap.release()
                ...
                if action.type == ActionType.MOUSE_MOVE:
                    ...
                    if vkb:
                        vkb.toggle()
                                     ...
                                     ActionType.DWELL_CLICK) and vkb and vkb.visible:
                    vkb.click_current()
                else:
                    if os_ctrl:
                        os_ctrl.execute(action)
    ...
    cap.release()
    if SHOW_CAMERA_WIN and has_gui_session():
